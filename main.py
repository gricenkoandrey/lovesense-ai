"""
LoveSenseAI v6 - main.py (full implementation)
Features:
- Trial mode (2 free AI requests per user)
- Conversion-focused Fast Checkout (demo instant grant)
- Manual payment flow with admin approval
- Referral system (referrer gets +7 days on referred paid purchase)
- Admin panel: stats, users, orders, premium management, marketing templates
- Optional OpenAI integration via OPENAI_API_KEY (set in Replit Secrets)
- Keepalive / health endpoint for uptime monitors
- Designed to run on Replit (long polling)

Set these Replit Secrets before running:
- BOT_TOKEN (required)
- ADMIN_IDS (comma-separated admin Telegram IDs)
Optional:
- OPENAI_API_KEY
- STRIPE_SECRET (if you want Stripe later)
- PING_URL (for keepalive pinger)
"""
...
PING_URL = os.getenv("PING_URL")  # optional (for keepalive pinger)
...
def kb_main(is_admin=False):
    ...
    if is_admin: kb.add(InlineKeyboardButton("🛠 Admin Panel", callback_data="admin_panel"))
    return kb
...
async def set_lang(cb: types.CallbackQuery):
    ...
    if code not in SUPPORTED_LANG: await cb.answer("Unsupported"); return
    u = get_user(cb.from_user.id); u["lang"]=code; save_user(u)
...
async def ai_generate(prompt:str, premium:bool=False):
    # if OPENAI_KEY is present, call OpenAI's chat completions
    if OPENAI_KEY:
        try:
            import aiohttp
    ...
    # fallback: local canned responses
    if premium:
...
async def catch_message(msg: types.Message):
    # Treat most user messages as AI prompts for demo
...
async def fast_checkout(cb: types.CallbackQuery):
    ...
    for admin in ADMIN_IDS:
        try: await bot.send_message(admin, f"[SALE] User {uid} bought Premium (instant demo).")
        except: pass
    await cb.answer()
...
async def grant_cb(cb: types.CallbackQuery):
    ...
    save_json(ORDERS_FILE, orders)
    try: await bot.send_message(uid, "Вам выдали Premium. Спасибо!")
    except: pass
...
async def reject_cb(cb: types.CallbackQuery):
    ...
    for o in orders:
        if o.get("user_id")==uid and o.get("status")=="pending_manual":
            o["status"]="rejected"
    save_json(ORDERS_FILE, orders)
    try: await bot.send_message(uid, "К сожалению, оплата не подтверждена.")
    except: pass
...
async def admin_stats_cb(cb: types.CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS: await cb.answer("Только админ."); return
    users = load_json(USERS_FILE)
    orders = load_json(ORDERS_FILE)
    total = len(users); active = sum(1 for u in users.values() if u.get("premium_until",0) > time.time())
    ...
    if sales_day:
        for d,c in sorted(sales_day.items(), reverse=True)[:10]:
            text += f"• {d}: <b>{c}</b>\n"
    else: text += "Нет подтверждённых продаж.\n"
    text += "\n💳 Продажи по месяцам:\n"
    if sales_month:
...
async def admin_orders_cb(cb: types.CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS: await cb.answer("Только админ."); return
    orders = load_json(ORDERS_FILE)
    if not orders: await cb.message.answer("Нет заказов."); await cb.answer(); return
    text = "💳 Заказы:\n"
...
async def admin_premium_cb(cb: types.CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS: await cb.answer("Только админ"); return
    await cb.message.answer("Используйте команды: /grant <id> или /revoke <id> (в чат боту)."); await cb.answer()

@dp.message(commands=['grant','revoke'])
async def grant_revoke_cmd(msg: types.Message):
    if msg.from_user.id not in ADMIN_IDS: return
    parts = msg.get_args().split()
    if not parts: await msg.answer('Usage: /grant <id> or /revoke <id>'); return
    try:
        uid = int(parts[0])
        u = get_user(uid)
        if msg.text.startswith('/grant'):
            u['premium_until'] = int(time.time()) + PREMIUM_DAYS*24*3600
            save_user(u); await msg.answer(f'Granted premium to {uid}'); 
        else:
            u['premium_until'] = 0; save_user(u); await msg.answer(f'Revoked premium for {uid}')
    except Exception as e:
...
async def admin_refs_cb(cb: types.CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS: await cb.answer('Только админ'); return
    users = load_json(USERS_FILE)
    sorted_refs = sorted(((u.get('ref_count',0), uid) for uid,u in users.items()), reverse=True)[:20]
    text = '🔗 Топ рефералов:\n' + '\n'.join(f'ID:{uid} — {cnt}' for cnt,uid in sorted_refs)
    await cb.message.answer(text); await cb.answer()
